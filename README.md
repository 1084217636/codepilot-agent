...
V2 已实现 `GET /health`、普通 JSON Chat、显式 LangGraph StateGraph、唯一的 `read_file` Tool、workspace 路径隔离、`CODEPILOT_DEBUG` 教学日志，以及 `POST /api/chat/stream` SSE 过程事件流。每个聊天请求都会显示独立 `request_id`、图的节点跳转、LLM 调用次数、消息类型、工具参数摘要与总耗时；不会输出 API Key、完整 Prompt 或完整文件内容。V2 不包含 RAG、代码修改、测试执行、数据库、Branch 或 DeerFlow Runtime。
