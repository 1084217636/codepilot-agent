...
`CODEPILOT_DEBUG` is a teaching feature. Its numbered logs must describe real
execution boundaries without emitting API keys, authentication headers, complete
prompts, or unbounded tool output. Keep `/health` independent of model settings
so learners can verify FastAPI before configuring a provider.
